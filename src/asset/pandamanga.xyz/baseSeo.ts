...
export default {
  ...
  canonical: `https://${_lbl_domain_name}`,
    ...
    manifest:{
        ...
        title:_sb_seo_df_title[Math.floor(Math.random() * _sb_seo_df_title.length)],
        description: _sb_seo_page_default_desc[Math.floor(Math.random() * _sb_seo_page_default_desc.length)],
        siteName: _sb_seo_df_title[Math.floor(Math.random() * _sb_seo_df_title.length)],
        images: [{
