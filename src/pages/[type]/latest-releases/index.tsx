 ...
 // console.log("_domain",_domain);
  ...
  const nameType =config_base?.SelectMangaTypeName(context.params.type);
    ...
    await Promise.all([
        ...
        CallApiWithRetry(config.apiPath + config.endPointPath.homeLastUpdate + _page, 9)

    ]).then((result) => {
        data_popular = result[0];
        data_lastuppdate = result[1];
    });
    ...
    console.log(`Last release ${context.req.url} :`,ex);
    return {
      props: {
        data_group:  null,
        id: null,
...
const LatestReleasePage = ({ type, data_popular, data_lastuppdate,config,page,domain}: any) => {

  //console.log('config ',config.configSetting);
  ...
  const _groupPageName="Last-release";
  const _pagePath='latest-releases';
    ...
    <>
     <Head>
      ...
      <meta name="robots" content={`${config_prefix_baseSeo._hostwww}/${domain}/robots.txt`}/>
        ...
        canonical={`/${config.typeManga}/${_pagePath}/latest-releases?page=${parseInt(data_lastuppdate.currentPage)}`}
        openGraph={{
          url: `/${config.typeManga}/${_pagePath}/latest-releases?page=${parseInt(data_lastuppdate.currentPage)}`,
          title: `${_groupPageName} latest-releases`,
          description: `${config?.configSetting?.sb_seo_page_group_desc
            .replace(/{domain}/g, config?.configSetting?.lbl_domain_name)
            .replace(/{groupname}/g, _groupPageName)
            .replace(/{key}/g, 'latest releases')
        ...
        additionalLinkTags={[{
          rel: "next",
          href: `/${config.typeManga}/${_pagePath}/latest-releases?page=${parseInt(data_lastuppdate.currentPage)+1}`,
        }, {
          rel: "prev",
          href: `/${config.typeManga}/${_pagePath}/latest-releases?page=${parseInt(data_lastuppdate.currentPage)-1}`,
        }
      ...
      />
      <h1 style={{ "display": "none" }}>{config?.configSetting?.lbl_Find_list} {`latest releases`}</h1>
            ...
            <div className="lg:pl-60 ">
                <main className=" bg-slate-900/60 border border-slate-700">
                    <div id="wapper" className="mt-4 px-2">
                        ...
                        <LastRelease typeManga={type} data_lastuppdate={data_lastuppdate}  config={config}/>
                ...
                {/* <ReactPaginate
                 ...
                 // pageRangeDisplayed={1} 
                  ...
                  pageCount={data_lastuppdate.totalPage}
                ...
                /> */}
                        ...
                        <PopupRelease typeManga={type} data_popular={data_popular} config={config}/>
                    </div>
                </main>
            </div>
        </>
