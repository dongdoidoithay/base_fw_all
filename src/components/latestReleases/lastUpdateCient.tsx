...
const FetchDataPage =  (config: any, pageParam: number) => {
  return  FetchApi(
    config.apiPath + config.endPointPath.homeLastUpdate + pageParam
  );
};
const LastReleaseClient = ({ typeManga,config }: any) => {
  const sectionRef = useRef<HTMLDivElement>(null);
  const [page, setPage] = useState(0);

  const data= FetchDataPage(config, page);
...
export default LastReleaseClient;
