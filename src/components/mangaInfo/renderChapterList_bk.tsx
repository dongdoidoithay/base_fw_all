...
const RenderChapterList = ({ id, config, mangaName, idchapter, dataManga, isSeo, domain }:any) => {
  //console.log(`seo_base domain`,domain);
  let config_base = configBase[domain];
  let seo_base = baseSeo[domain];
   //console.log(`seo_base`,seo_base);
 ...
 /*  const [dataChapter, SetDataChapter] = useState(null); */
  //const dataChapter = FetchDataChaper(config, _fixid, idDetailFilter, sort, currentPage);
  /* useEffect(() => {
    async function fetchData() {
      FetchApi(config.apiPath + config.endPointPath.infoChapter +_fixid +"/16/" +currentPage +"/" +idDetailFilter +"/" +sort).then((data)=>{
       // console.log(`data ${data}`);
        SetDataChapter(data);
      });    
    }
    fetchData();
  }, [_fixid,currentPage,idDetailFilter]) */
  const {
  ...
  if (isSeo) {
    ...
    const url = `${config.configPrefix.pageManga}/${config.configPrefix.startManga}${dataManga.idDoc}${config.configPrefix.endManga}`;
    ...
    if(!isFetchingChapter &&
      ...
      dataChapter.data.map((item: any, index: number) => {
        const url_part = `${config.configPrefix.pageViewManga}/${config.configPrefix.startManga}${item.idDoc}${config.configPrefix.endManga}/${config.configPrefix.startViewmanga}${item.idDetail}${config.configPrefix.endViewmanga}`;
        const _itenpart = {
          "@type": "ComicIssue",
    ...
    json_chapter_ld = {
      ...
      "itemListElement": [
        ...
        },
        {
          ...
          "position": 2,
          "name": "Type Manga",
          "item": `${config.configPrefix.pageManga}`
        },
        {
          "@type": "ListItem",
          "position": 3,
          "name": mangaName,
          "item": `${config.configPrefix.pageManga}/${config.configPrefix.startManga}${_fixid}${config.configPrefix.endManga}`
        },
        ...chapter_js
      ]
    }
          ...
          {!isFetchingChapter &&
            ...
            dataChapter.data.map((item: any, index: number) => {
              ...
              return (
                  ...
                  >
                    <a
                      href={`${config.configPrefix.pageViewManga}/${config.configPrefix.startManga}${item.idDoc}${config.configPrefix.endManga}/${config.configPrefix.startViewmanga}${item.idDetail}${config.configPrefix.endViewmanga}`}
