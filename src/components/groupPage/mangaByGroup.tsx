...
import {FetchGroupDataClient } from '@/constants/FetchApi';
...
const MangaByGroup = ({ typeManga, typeApi, idFind, pageIndex,config}:any) => {
 ...
 // const data=FetchData(config, typeApi, idFind, page);
  const [data,SetData]=useState(null);
  useEffect(() => {
    SetData(null);
    async function fetchData() {
      FetchGroupDataClient(config,typeApi,idFind,page).then((_data)=>{
    ...
    <div ref={sectionRef}>
      <Boundary labels={`Result - ${idFind}`} />
       <table className="w-full text-left border-collapse text-sm ">
        <tbody>
            {!data && tableSkeleton()}
            { data && data.data && data.data.map((data: any, index: number) => (_renderItem(data, index)))}
        </tbody>
        </table>
        {/*load more*/}
        {data&& data.data && data.data.length>0 && PageAction()}
    </div>
  );
}
